...
static double capture_variant(cumes::EquilibriumOperator<Real>& eq,
                              ...
                              cumes::CudaGraph* out) {
    double t0 = now_us();
    *out = cumes::CudaGraph::capture(
        stream, [&]() { eq.enqueue(iter, iter2, sched, stream); });
    return now_us() - t0;
}
...
int main(int argc, char** argv) {
    ...
    int warmup = 5;
    int passes = 200;
    const int sync_period =
        50;  // bound the async queue during submission timing
    ...
    for (int i = 1; i < argc; ++i) {
        if (const char* v = args.need(i, "config"))
            ...
            fprintf(stderr, "graph_realpass: unknown option '%s'\n", argv[i]);
            return 2;
        }
    }
    if (passes < 1 || warmup < 0) {
        fprintf(stderr, "graph_realpass: bad --passes/--warmup\n");
        return 2;
    }
    ...
    const int ret = cumes::run_in_stage_arena<
        Real>(p, [&](cumes::DeviceArena& arena) -> int {
        ...
        for (int w = 0; w < warmup; ++w) {
            ...
            cumes::check_cuda(
                cudaMemcpyAsync(h_ctl.data(), equilibrium.control_device(),
        ...
        bool param_update_works = false;
        {
            ...
            // Functional gate: the exec-side scalar update must change the
            // output.
            auto run_and_read = [&]() {
            ...
            printf(
                "gate_setparams_zeroZ     : %s (zeroZ=1 -> %d, zeroZ=0 -> "
                "%d)\n",
        ...
        // ---- gate 1: graph replay == direct pass, bitwise ----
        {
            ...
            stream.synchronize();
            Real h_direct[16];
            cudaMemcpy(h_direct, equilibrium.control_device(),
            ...
            double max_diff = 0.0;
            for (int i = 0; i < 16; ++i)
                max_diff = std::max(max_diff, std::fabs((double)h_direct[i] -
                                                        (double)h_graph[i]));
            printf(
                "gate_graph_vs_direct     : %s (max |diff| = %.3e over 16 "
                "control scalars)\n",
        ...
        const double cap_base_us = capture_variant(
            equilibrium, iter, iter2, base_sched, stream.get(), &g_base);
        const double cap_refresh_us = capture_variant(
            equilibrium, iter, iter2, refresh_sched, stream.get(), &g_refresh);
        const double cap_zeroz_us = capture_variant(
            equilibrium, iter, iter2, zeroz_sched, stream.get(), &g_zeroz);
        stream.synchronize();
        ...
        std::vector<double> prod_direct =
            production_loop(false, &gpu_direct_us);
        ...
        printf(
            "capture+instantiate_us   : base %.1f | refresh %.1f | zeroZ "
            "%.1f\n",
            cap_base_us, cap_refresh_us, cap_zeroz_us);
        printf(
            "submission_us_per_pass   : enqueue %.2f | graph launch %.2f | "
            "saving %.2f\n",
            median(enc_us), median(lch_us), median(enc_us) - median(lch_us));
        printf(
            "production_wall_us_pass  : direct %.2f | graph %.2f | saving "
            "%.2f\n",
            median(prod_direct), median(prod_graph),
            median(prod_direct) - median(prod_graph));
        printf("gpu_us_per_pass          : direct %.2f | graph %.2f\n",
               gpu_direct_us / passes, gpu_graph_us / passes);
        printf(
            "setparams_us_per_update  : %.2f (zeroZ scalar, m1Constraint "
            "node)\n",
            setparams_us);
        ...
        if (out_path) {
            ...
            if (!f) {
                fprintf(stderr, "graph_realpass: cannot open --out %s\n",
                        out_path);
                return 2;
            }
            fprintf(
                ...
                "  \"gpu\": \"%s\",\n"
                "  \"sm\": \"%d%d\",\n"
                "  \"capture_us\": {\"base\": %.1f, \"refresh\": %.1f, "
                "\"zeroz\": %.1f},\n"
                "  \"submission_us_per_pass\": {\"enqueue\": %.2f, "
                "\"graph_launch\": %.2f, \"saving\": %.2f},\n"
                "  \"production_wall_us_per_pass\": {\"direct\": %.2f, "
                "\"graph\": %.2f, \"saving\": %.2f},\n"
                ...
                prop.name, prop.major, prop.minor, cap_base_us, cap_refresh_us,
                cap_zeroz_us, median(enc_us), median(lch_us),
                median(enc_us) - median(lch_us), median(prod_direct),
                median(prod_graph), median(prod_direct) - median(prod_graph),
                gpu_direct_us / passes, gpu_graph_us / passes, setparams_us,
                param_update_works ? "true" : "false");
            fclose(f);
