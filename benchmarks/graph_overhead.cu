...
//
// The solver enqueues roughly 24 submissions per evaluated pass
// (extrapolateAxis, inverseDFTFused + cuFFT, computeGeometry,
// computeJacobianStats, computeForces, constraintCompute, forwardDFT + cuFFT,
// scalxc, m1Constraint, two residual reductions, m1PreconScale, preconApply,
// the control copy, …), so K=24 is the representative count. Combine the
// numbers here with cumes_benchmark_fixed_ iteration's µs/iter to decide
// "select by measured shape".
//
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (const char* v = args.need(i, "kernels"))
            k = atoi(v);
        else if (const char* v = args.need(i, "reps"))
            reps = atoi(v);
        else {
            fprintf(stderr, "graph_overhead: unknown option '%s'\n", argv[i]);
            return 2;
        }
    }
