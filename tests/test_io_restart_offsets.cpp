...
// ---------------------------------------------------------------------------
#ifdef CUMES_HAVE_NETCDF
static bool write_netcdf_fixture(const std::string& path,
                                 const std::vector<int>& rst_off,
                                 int nrestarts,
                                 bool wrong_iter_extent) {
    int ncid = -1;
...
#define NC_F(expr)                            \
    ...
    const bool ok = [&]() -> bool {
        ...
        // Wrong-extent case: declare restart_iteration against nstages instead
        // of nrestarts — the extent check must reject it before any indexing.
        ...
        std::vector<int> rst_iter(
            (size_t)(wrong_iter_extent ? nstages : nrestarts), 7);
...
#ifdef CUMES_HAVE_HDF5
static bool write_hdf5_fixture(const std::string& path,
                               const std::vector<int>& rst_off,
                               int nrestarts,
                               bool wrong_iter_extent) {
    hid_t fid =
...
#define H5_F(expr)                    \
    ...
    const bool ok = [&]() -> bool {
        ...
        for (int c = 0; c < 6; ++c) {
            ...
            hid_t ds = H5Dcreate2(fid, fams[c], H5T_NATIVE_DOUBLE, sp,
        ...
        // the datasets, so the mismatch is manufactured on a stage array
        // instead.
        std::vector<int> stage_iter(
            (size_t)(wrong_iter_extent ? nstages + 1 : nstages), 1);
        std::vector<int> stage_conv((size_t)nstages, 1);
        std::vector<double> stage_res((size_t)nstages, 0.0);
...
// ---------------------------------------------------------------------------
static void test_backend(const char* name,
    ...
    auto run_case = [&](const char* label, const std::vector<int>& offsets,
        ...
        if (expect_ok) {
            ...
            check(false,
                  format("{}: {} rejected with typed failure", name, label));
        }
    };
...
#ifdef CUMES_HAVE_NETCDF
    test_backend("netcdf", write_netcdf_fixture, OutputFormat::NETCDF, ".nc",
                 dir);
#else
    std::cout << "SKIP netcdf corrupted-offset cases (backend not compiled)\n";
#endif
#ifdef CUMES_HAVE_HDF5
    test_backend("hdf5", write_hdf5_fixture, OutputFormat::HDF5, ".h5", dir);
#else
