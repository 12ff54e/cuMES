...
static void testScaling() {
    ...
    runConstraint<T>(T(0.0), f0e.data(), f0z.data(), t0.data(), ns, mnmax, nZnT);
    runConstraint<T>(T(1.0), f1e.data(), f1z.data(), t1.data(), ns, mnmax, nZnT);
    runConstraint<T>(T(2.0), f2e.data(), f2z.data(), t2.data(), ns, mnmax, nZnT);
