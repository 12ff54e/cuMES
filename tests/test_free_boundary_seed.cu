...
int main(int argc, char** argv) {
    ...
    // Per-stage edge pressure (mirrors StageSolver::run).
    {
        ...
        const double mass_edge =
            cumes::eval_mass_profile<double>(vp.spec(), 1.0);
        double edge_pressure = cumes::eval_mass_profile<double>(
    ...
    try {
        result = solver_run<double>(storage, p, profiles, transform, rs,
                                    geometry, std::nullopt, stream,
                                    std::nullopt, std::nullopt, std::ref(vac));
