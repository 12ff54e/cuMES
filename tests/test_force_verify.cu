...
static void cpuForwardProject(
    const std::vector<double>& armn_e, const std::vector<double>& armn_o,
    const std::vector<double>& azmn_e, const std::vector<double>& azmn_o,
    const std::vector<double>& brmn_e, const std::vector<double>& brmn_o,
    const std::vector<double>& bzmn_e, const std::vector<double>& bzmn_o,
    const std::vector<double>& crmn_e, const std::vector<double>& crmn_o,
    const std::vector<double>& czmn_e, const std::vector<double>& czmn_o,
    const std::vector<double>& blmn_e, const std::vector<double>& blmn_o,
    const std::vector<double>& clmn_e, const std::vector<double>& clmn_o,
    int ns, int mpol, int ntor, int nfp, int ntheta, int nzeta, int nZnT,
        ...
        for (int j = 0; j < ns; ++j) {
            ...
            for (int k = 0; k < nzeta; ++k) {
                ...
                for (int l = 0; l < nThetaRed; ++l) {
                    ...
                    const double mcos = w * m * cos(m * th), msin = w * (-m) * sin(m * th);
                ...
                vs[0][k] = v0; vs[1][k] = v1; vs[2][k] = v2; vs[3][k] = v3;
                vs[4][k] = v4; vs[5][k] = v5; vs[6][k] = v6; vs[7][k] = v7;
                vs[8][k] = v8; vs[9][k] = v9; vs[10][k] = v10; vs[11][k] = v11;
            }
...
static ForceGate runForceGate(cumes::SpectralStorage<double>& storage,
    ...
    frcon_e.zero(); frcon_o.zero(); fzcon_e.zero(); fzcon_o.zero();
    ...
    cc(cudaMemcpy(h_f.data(), d_f_spec.data(), n6 * sizeof(double), cudaMemcpyDeviceToHost), "cpy f");
    for (int c = 0; c < 6; ++c) {
        double sum = 0;
        for (size_t i = 0; i < (size_t)p.ns * p.mnmax; ++i)
            sum += h_f[c * (size_t)p.ns * p.mnmax + i] * h_f[c * (size_t)p.ns * p.mnmax + i];
        sum /= (double)(p.ns * p.mnmax);
        if (c == 0 || c == 3) g.fsqr_prod += sum;
        else if (c == 1 || c == 4) g.fsqz_prod += sum;
        else if (c == 2 || c == 5) g.fsql_prod += sum;
    }

    // ---- Independent CPU recompute on the production real-space state ----
    auto get = [&](const double* d, size_t n) {
        std::vector<double> v(n);
        cc(cudaMemcpy(v.data(), d, n * sizeof(double), cudaMemcpyDeviceToHost), "get");
        return v;
    };
    // production force outputs (the comparison target)
    std::vector<double> g_armn_e = get(rs.d_armn_e, nF), g_armn_o = get(rs.d_armn_o, nF);
    std::vector<double> g_azmn_e = get(rs.d_azmn_e, nF), g_azmn_o = get(rs.d_azmn_o, nF);
    std::vector<double> g_brmn_e = get(rs.d_brmn_e, nF), g_brmn_o = get(rs.d_brmn_o, nF);
    std::vector<double> g_bzmn_e = get(rs.d_bzmn_e, nF), g_bzmn_o = get(rs.d_bzmn_o, nF);
    std::vector<double> g_crmn_e = get(rs.d_crmn_e, nF), g_crmn_o = get(rs.d_crmn_o, nF);
    std::vector<double> g_czmn_e = get(rs.d_czmn_e, nF), g_czmn_o = get(rs.d_czmn_o, nF);
    std::vector<double> g_blmn_e = get(rs.d_blmn_e, nF), g_blmn_o = get(rs.d_blmn_o, nF);
    std::vector<double> g_clmn_e = get(rs.d_clmn_e, nF), g_clmn_o = get(rs.d_clmn_o, nF);
    // full-grid parity geometry
    std::vector<double> r_e = get(rs.d_r_e, nF), r_o = get(rs.d_r_o, nF), z_o = get(rs.d_z_o, nF);
    ...
    std::vector<double> r12 = get(hg.r12.data(), nH), ru12 = get(hg.ru12.data(), nH);
    std::vector<double> zu12 = get(hg.zu12.data(), nH), rs_h = get(hg.rs.data(), nH);
    std::vector<double> zs_h = get(hg.zs.data(), nH), tau = get(hg.tau.data(), nH);
    std::vector<double> gsqrt = get(hg.gsqrt.data(), nH), guv = get(hg.guv.data(), nH);
    std::vector<double> gvv = get(hg.gvv.data(), nH);
    auto mf = geometry.magnetic_field_views(p);
    std::vector<double> bsupu = get(mf.bsupu.data(), nH), bsupv = get(mf.bsupv.data(), nH);
    std::vector<double> bsubu = get(mf.bsubu.data(), nH), bsubv = get(mf.bsubv.data(), nH);
    std::vector<double> totalP = get(mf.total_pressure.data(), nH);
    // profiles
    std::vector<double> sqrtS_F = get(rp.sqrtS_F, p.ns), sqrtS_H = get(rp.sqrtS_H, p.ns - 1);
    ...
    cpuForces(r_e, r_o, z_o, ru_e, ru_o, zu_e, zu_o, rv_e, rv_o, zv_e, zv_o,
              ...
              p.ns, p.nZnT, p.lamscale, profiles.delta_s(),
    ...
    std::vector<double> spec;
    cpuForwardProject(c_armn_e, c_armn_o, c_azmn_e, c_azmn_o, c_brmn_e, c_brmn_o,
                      c_bzmn_e, c_bzmn_o, c_crmn_e, c_crmn_o, c_czmn_e, c_czmn_o,
                      c_blmn_e, c_blmn_o, c_clmn_e, c_clmn_o,
                      p.ns, p.mpol, p.ntor, p.nfp, p.ntheta, p.nzeta, p.nZnT,
                      spec);
    for (int c = 0; c < 6; ++c) {
        double sum = 0;
        for (size_t i = 0; i < (size_t)p.ns * p.mnmax; ++i)
            sum += spec[c * (size_t)p.ns * p.mnmax + i] * spec[c * (size_t)p.ns * p.mnmax + i];
        sum /= (double)(p.ns * p.mnmax);
        if (c == 0 || c == 3) g.fsqr_cpu += sum;
        else if (c == 1 || c == 4) g.fsqz_cpu += sum;
        else if (c == 2 || c == 5) g.fsql_cpu += sum;
    }
    return g;
}
...
int main() {
    ...
    auto getFam = [&](cumes::SpectralComponent c) {
        ...
        cc(cudaMemcpy(v.data(), storage.family_ptr(c), nb, cudaMemcpyDeviceToHost), "get family");
        return v;
    };
    ...
    cc(cudaMemcpy(storage.family_ptr(cumes::SpectralComponent::Rcc), h_rmncc_c.data(), nb, cudaMemcpyHostToDevice), "restore rmncc");
    cc(cudaMemcpy(storage.family_ptr(cumes::SpectralComponent::Zsc), h_zmnsc_c.data(), nb, cudaMemcpyHostToDevice), "restore zmnsc");
    cc(cudaMemcpy(storage.family_ptr(cumes::SpectralComponent::Lsc), h_lmnsc_c.data(), nb, cudaMemcpyHostToDevice), "restore lmnsc");
    cc(cudaMemcpy(storage.family_ptr(cumes::SpectralComponent::Rss), h_rmnss_c.data(), nb, cudaMemcpyHostToDevice), "restore rmnss");
