...
static void loadState(SpectralState& st, const GridParams& p, const char* base) {
    ...
    auto rd = [&](double* d, const char* fn) {
        ...
        fread(h, sizeof(double), p.ns * p.mnmax, fp);
        fclose(fp);
...
int main() {
    ...
    for (int i = 0; i < 8; ++i)
        cudaMemcpy(&hb[i], mw.d_bsupu + 50 * nZnT + ks[i], 8, cudaMemcpyDeviceToHost);
    int nz = 0;
    double* h_all = new double[(p.ns - 1) * nZnT];
    cudaMemcpy(h_all, mw.d_bsupu, (p.ns - 1) * nZnT * 8, cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(h_all, mw.d_bsubu, (p.ns - 1) * nZnT * 8, cudaMemcpyDeviceToHost);
    int nz2 = 0;
