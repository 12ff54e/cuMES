...
template void preconCompute<float>(const cumes::RealSpaceStorage<float>&, const FourierPlan<float>&, const GridParams<float>&, const RadialProfiles<float>&, const MetricWorkspace<float>&, PreconWorkspace<float>&, cudaStream_t);
