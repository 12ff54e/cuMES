...
// Included once per scalar type by axisymmetric_double.cu / axisymmetric_float.cu
// (the explicit-instantiation split, one scalar type per TU).
//
// The axisymmetric backend replaces the cuFFT length-one Z2D/D2Z round trip with
// direct poloidal synthesis/projection. For ntor=0, nzeta=1 every folded mode
// has n=0, so (from the generic pack kernels' n=0 branch) the only nonzero
...
__global__ void inverseKernel(
    ...
    T* __restrict__ r_e, T* __restrict__ z_e, T* __restrict__ l_e,
    T* __restrict__ ru_e, T* __restrict__ zu_e, T* __restrict__ lu_e,
    T* __restrict__ r_o, T* __restrict__ z_o, T* __restrict__ l_o,
    T* __restrict__ ru_o, T* __restrict__ zu_o, T* __restrict__ lu_o,
    T* __restrict__ rv_e, T* __restrict__ zv_e, T* __restrict__ lv_e,
    T* __restrict__ rv_o, T* __restrict__ zv_o, T* __restrict__ lv_o) {
  ...
  for (int m = 0; m < mpol; ++m) {
    ...
    if (m % 2 == 1) {
      ro += rv; ruo += ruv; zo += zv; zuo += zuv; lo += lv; luo += luv;
    } else {
      re += rv; rue += ruv; ze += zv; zue += zuv; le += lv; lue += luv;
    }
  }
  const int idx = j * nZnT + l;  // nZnT == ntheta (nzeta == 1)
  r_e[idx] = re; ru_e[idx] = rue; rv_e[idx] = T(0);
  z_e[idx] = ze; zu_e[idx] = zue; zv_e[idx] = T(0);
  l_e[idx] = le; lu_e[idx] = lue; lv_e[idx] = T(0);
  r_o[idx] = ro; ru_o[idx] = ruo; rv_o[idx] = T(0);
  z_o[idx] = zo; zu_o[idx] = zuo; zv_o[idx] = T(0);
  l_o[idx] = lo; lu_o[idx] = luo; lv_o[idx] = T(0);
}
...
__global__ void forwardKernel(
    ...
    int ns, int mpol, int ntheta, int nThetaRed, int nZnT,
...
__global__ void rzconKernel(
    SpectralView<const T, PhysicalStateDomain> coeff, int ns, int mpol,
    int ntheta, int nZnT, const T* __restrict__ cos_th,
    const T* __restrict__ sin_th, T* __restrict__ rCon, T* __restrict__ zCon) {
  const int j = blockIdx.x;
  const int l = blockIdx.y * blockDim.x + threadIdx.x;
  if (l >= ntheta) return;
  T r = 0, z = 0;
...
template <class T>
__global__ void dealiasKernel(
    const T* __restrict__ gConEff, const T* __restrict__ tcon,
    const T* __restrict__ faccon, const T* __restrict__ sin_th, int ns, int mpol,
    int ntheta, int nZnT, T* __restrict__ gCon) {
...
AxisymmetricOperator<T>::AxisymmetricOperator(const DeviceParams<T>& p) : p_(p) {
  ...
  check_cuda(cudaMemcpy(msin_th_.data(), h_msin.data(),
             ...
             "axisym msin_th");
  check_cuda(cudaMemcpy(fwd_w_.data(), h_fwd.data(),
                        h_fwd.size() * sizeof(T), cudaMemcpyHostToDevice),
             "axisym fwd_w");
}
...
void AxisymmetricOperator<T>::enqueue_forward(
    ForceParityViews<const T> f, ConstraintForceViews<const T> cf,
    SpectralView<T, DecomposedResidualDomain> residual, cudaStream_t stream) {
  ...
  axisym_detail::forwardKernel<T><<<grid, 1, 0, stream>>>(
      ...
      mcos_th_.data(), msin_th_.data(), fwd_w_.data(), p_.ns, p_.mpol, p_.ntheta,
      nThetaRed, p_.nZnT, residual);
  check_cuda(cudaGetLastError(), "axisym forward");
}

template <class T>
void AxisymmetricOperator<T>::enqueue_rzcon(
    SpectralView<const T, PhysicalStateDomain> coefficients, RealFieldView<T> rCon,
    RealFieldView<T> zCon, cudaStream_t stream) {
...
template <class T>
void AxisymmetricOperator<T>::enqueue_dealias(
    RealFieldView<const T> gConEff, const T* tcon, const T* faccon,
    RealFieldView<T> gCon, cudaStream_t stream) {
  const int ntheta = p_.ntheta;
  const int blk = 32;
  const dim3 grid(p_.ns, (ntheta + blk - 1) / blk);
