struct Params {
    ...
    norm_hi:f32,norm_lo:f32,sqrt2_hi:f32,sqrt2_lo:f32,lasym:u32,
};
...
fn pack(@builtin(global_invocation_id) id:vec3u){
    let tr=select(params.ntheta/2u+1u,params.ntheta,params.lasym!=0u);let count=20u*params.ns*tr*params.nzeta;
...
fn unpack(@builtin(global_invocation_id) id:vec3u){
    let tr=select(params.ntheta/2u+1u,params.ntheta,params.lasym!=0u);let plane=20u*params.ns*tr*(params.ntor+1u);
