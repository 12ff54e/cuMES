...
// Fourteen forward-transform input fields.
...
fn main(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    for (var field = 0u; field < 10u; field++) {
        put(field, point, force.data[field * params.points + point]);
    }
    let surface = point / params.ntheta;
    if (surface == 0u) {
        for (var field = 10u; field < 14u; field++) { put(field, point, 0.0); }
        return;
    }
    ...
    put(10u, point, ru * gc);
    put(11u, point, ru * gc * sqrt_s);
    put(12u, point, zu * gc);
    put(13u, point, zu * gc * sqrt_s);
}
