...
__global__ void forces_kernel(cumes::GeometryParityViews<T> full,
    ...
    T* d_armn_e = force.armn_e.data();
    T* d_armn_o = force.armn_o.data();
    T* d_azmn_e = force.azmn_e.data();
    T* d_azmn_o = force.azmn_o.data();
    T* d_brmn_e = force.brmn_e.data();
    T* d_brmn_o = force.brmn_o.data();
    T* d_bzmn_e = force.bzmn_e.data();
    T* d_bzmn_o = force.bzmn_o.data();
    T* d_crmn_e = force.crmn_e.data();
    T* d_crmn_o = force.crmn_o.data();
    T* d_czmn_e = force.czmn_e.data();
    T* d_czmn_o = force.czmn_o.data();
    T* d_blmn_e = force.blmn_e.data();
    T* d_blmn_o = force.blmn_o.data();
    T* d_clmn_e = force.clmn_e.data();
    T* d_clmn_o = force.clmn_o.data();
    ...
    T armn_e = (zup_o - zup_i) * inv_ds + T(0.5) * (taup_o + taup_i) -
               gbvbv_avg * re_j - gbvbv_wavg * ro_j;

    T armn_o = (zup_o * sH_o - zup_i * sH_i) * inv_ds -
               T(0.5) * P_wavg * zue_j - T(0.5) * P_avg * zuo_j +
               T(0.5) * (taup_o * sH_o + taup_i * sH_i) - gbvbv_wavg * re_j -
               gbvbv_avg * ro_j * sFull;
    ...
    T brmn_e = T(0.5) * (zsp_o + zsp_i) + T(0.5) * P_wavg * zo_j -
               gbubu_avg * rue_j - gbubu_wavg * ruo_j - gbubv_avg * rve_j -
               gbubv_wavg * rvo_j;

    T brmn_o = T(0.5) * (zsp_o * sH_o + zsp_i * sH_i) + T(0.5) * P_avg * zo_j -
               gbubu_wavg * rue_j - gbubu_avg * ruo_j * sFull -
               gbubv_wavg * rve_j - gbubv_avg * rvo_j * sFull;

    // ---- Poloidal Z-force (bzmn) ---------------------------------------
    T bzmn_e = -T(0.5) * (rsp_o + rsp_i) - T(0.5) * P_wavg * ro_j -
               gbubu_avg * zue_j - gbubu_wavg * zuo_j - gbubv_avg * zve_j -
               gbubv_wavg * zvo_j;

    T bzmn_o = -T(0.5) * (rsp_o * sH_o + rsp_i * sH_i) - T(0.5) * P_avg * ro_j -
               gbubu_wavg * zue_j - gbubu_avg * zuo_j * sFull -
               gbubv_wavg * zve_j - gbubv_avg * zvo_j * sFull;

    // ---- Toroidal forces (crmn/czmn, 3D) --------------------------------
    T crmn_e = gbubv_avg * rue_j + gbubv_wavg * ruo_j + gbvbv_avg * rve_j +
               gbvbv_wavg * rvo_j;
    T crmn_o = gbubv_wavg * rue_j + gbubv_avg * ruo_j * sFull +
               gbvbv_wavg * rve_j + gbvbv_avg * rvo_j * sFull;
    T czmn_e = gbubv_avg * zue_j + gbubv_wavg * zuo_j + gbvbv_avg * zve_j +
               gbvbv_wavg * zvo_j;
    ...
    // ---- Store ----------------------------------------------------------
    d_armn_e[idx_f] = armn_e;
    d_armn_o[idx_f] = armn_o;
    d_azmn_e[idx_f] = azmn_e;
    d_azmn_o[idx_f] = azmn_o;
    d_brmn_e[idx_f] = brmn_e;
    d_brmn_o[idx_f] = brmn_o;
    d_bzmn_e[idx_f] = bzmn_e;
    d_bzmn_o[idx_f] = bzmn_o;
    d_crmn_e[idx_f] = crmn_e;
    d_crmn_o[idx_f] = crmn_o;
    d_czmn_e[idx_f] = czmn_e;
    d_czmn_o[idx_f] = czmn_o;
    d_blmn_e[idx_f] = blmn_e;
    d_blmn_o[idx_f] = blmn_o;
    d_clmn_e[idx_f] = clmn_e;
    d_clmn_o[idx_f] = clmn_o;
}

template <typename T>
void cumes::ForceOperator<T>::enqueue(
    const cumes::RealSpaceStorage<T>& rs,
    const DeviceParams<T>& p,
