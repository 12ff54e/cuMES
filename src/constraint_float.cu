...
template void constraintCompute<float>(const DeviceParams<float>&, const cumes::RealSpaceStorage<float>&, const FourierPlan<float>&, const PreconWorkspace<float>&, ConstraintWorkspace<float>&, const float*, bool, cudaStream_t);
