...
template void constraintRzConCompute<float>(const GridParams<float>&, const FourierPlan<float>&, cumes::SpectralView<const float, cumes::PhysicalStateDomain>, ConstraintWorkspace<float>&, const float*);
