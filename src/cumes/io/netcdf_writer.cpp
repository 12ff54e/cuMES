...
#define NC_CHECK(rc_, msg_)                                           \
    do {                                                              \
        ...
        NC_CHECK(putStrAttr(ncid, "build_type", report.build.build_type) == true
                 ...
                 "attr build_type");
        NC_CHECK(
            putStrAttr(ncid, "scalar_type", report.build.scalar_type) == true
                ? NC_NOERR
                : NC_EATTMETA,
            "attr scalar_type");
        ...
        NC_CHECK(putStrAttr(ncid, "compile_flags",
        ...
        NC_CHECK(
            ...
            "attr source_path");
        NC_CHECK(
            putStrAttr(ncid, "source_hash", report.input.source_hash) == true
                ? NC_NOERR
                : NC_EATTMETA,
            "attr source_hash");
...
class NetcdfV1Reader final : public Reader {
   public:
    Result<EquilibriumSnapshot> read(const std::string& path,
        ...
        try {
            auto getDim = [&](const char* name, int& id_out,
                              size_t& len_out) -> bool {
                int id = -1;
            ...
            auto readStateFamily = [&](const char* name, int ns_dim,
            ...
            size_t ns = 0, mnmax = 0;
            if (!getDim("ns", ns_dim, ns) ||
                !getDim("mnmax", mnmax_dim, mnmax)) {
            ...
            if (report) {
                ...
                std::vector<double> st_fsqr(nstages), st_fsqz(nstages),
                    st_fsql(nstages);
                std::vector<int> rst_iter(nrestarts);
                if (!readVectorInt("stage_ns", nstages_dim, nstages,
                                   stage_ns) ||
                ...
                // read gets the exact-rank/type/extent hardening above.
                {
                    ...
                    if (nc_inq_varid(ncid, "mpol", &probe) == NC_NOERR) {
                        ...
                        // present one must pass the strict read.
                        {
                            ...
                            int makegrid_varid = -1;
                            if (nc_inq_varid(ncid, "makegrid_paramters_present",
                                             &makegrid_varid) == NC_NOERR) {
                                if (!readScalarInt("makegrid_paramters_present",
                                                   makegrid_present)) {
                                    return fail(
                                        "malformed embedded input record");
                                }
                                if (makegrid_present != 0) {
                                    ...
                                    if (!readScalarInt(
                                            ...
                                            normalize) ||
                                        !readScalarInt("makegrid_assume_"
                        ...
                        if (!readVectorInt("rbc_m", dim_nrbc, nrbc, ip.rbc_m) ||
                            ...
                            !readVectorDouble("zbs_value", dim_nzbs, nzbs,
                                              ip.zbs_value) ||
                            !readMatrixDouble("rbcc", dim_mpol, dim_ntorp1,
                                              mpol, ntorp1, ip.rbcc) ||
                            !readMatrixDouble("rbss", dim_mpol, dim_ntorp1,
                                              mpol, ntorp1, ip.rbss) ||
                            !readMatrixDouble("zbsc", dim_mpol, dim_ntorp1,
                                              mpol, ntorp1, ip.zbsc) ||
                            !readMatrixDouble("zbcs", dim_mpol, dim_ntorp1,
                                              mpol, ntorp1, ip.zbcs)) {
                            return fail("malformed embedded input record");
                        }
