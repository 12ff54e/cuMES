...
// ---- §8.10 split prototype -----------------------------------------------
// The R/Z force families only. Same arithmetic as the monolith's armn/azmn/
// brmn/bzmn/crmn/czmn block (copied verbatim so the per-family values are
// bit-identical); the lambda working set (lu_e/lu_o, bsubu/bsubv, phip_F) is
// not loaded, so the live register set is smaller.
template <typename T>
__global__ void rzForcesKernel(
    cumes::GeometryParityViews<T> full,
    cumes::BaseGeometryHalfViews<T> base,
    cumes::MagneticFieldViews<T> field,
    cumes::RadialProfileViews<T> radial,
    ...
    int ns, int nZnT, T delta_s)
{
    ...
    const T* r12 = base.r12.data(); const T* ru12 = base.ru12.data();
    const T* zu12 = base.zu12.data();
    const T* rs = base.rs.data(); const T* zs = base.zs.data();
    const T* tau = base.tau.data();
    const T* gsqrt = base.gsqrt.data();
    const T* guv = base.guv.data();
    const T* gvv = base.gvv.data();
    ...
    T r12_i=T(0), ru12_i=T(0), zu12_i=T(0), rs_i=T(0), zs_i=T(0), tau_i=T(0);
    T gsqrt_i=T(0), bsupu_i=T(0), bsupv_i=T(0), totalP_i=T(0);
    T sH_i = T(0), gvv_i = T(0), guv_i = T(0);

    T r12_o=T(0), ru12_o=T(0), zu12_o=T(0), rs_o=T(0), zs_o=T(0), tau_o=T(0);
    T gsqrt_o=T(0), bsupu_o=T(0), bsupv_o=T(0), totalP_o=T(0);
    T sH_o = T(0), gvv_o = T(0), guv_o = T(0);
...
// The lambda force families (blmn/clmn) only. Same arithmetic as the monolith's
// blmn/clmn block, copied verbatim; the R/Z working set is not loaded.
template <typename T>
__global__ void lambdaForcesKernel(
    cumes::GeometryParityViews<T> full,
    cumes::BaseGeometryHalfViews<T> base,
    cumes::MagneticFieldViews<T> field,
    cumes::RadialProfileViews<T> radial,
    cumes::ForceParityViews<T> force,
    T lamscale, int ns, int nZnT)
{
    const T* lu_e = full.lu_e.data(); const T* lu_o = full.lu_o.data();
    ...
    T gvv_i=T(0), gvv_o=T(0), gsqrt_i=T(0), gsqrt_o=T(0);
    T guv_i=T(0), guv_o=T(0), bsupu_i=T(0), bsupu_o=T(0);
    T bsubu_i=T(0), bsubu_o=T(0), bsubv_i=T(0), bsubv_o=T(0);
    T sH_i=T(0), sH_o=T(0);
    if (j > 0) {
        int h_i = k + (j - 1) * nZnT;
        gvv_i=gvv[h_i]; gsqrt_i=gsqrt[h_i]; guv_i=guv[h_i];
        bsupu_i=bsupu[h_i]; bsubu_i=bsubu[h_i]; bsubv_i=bsubv[h_i];
        sH_i = sqrtS_H[j - 1];
    }
    if (j < ns - 1) {
        int h_o = k + j * nZnT;
        gvv_o=gvv[h_o]; gsqrt_o=gsqrt[h_o]; guv_o=guv[h_o];
        bsupu_o=bsupu[h_o]; bsubu_o=bsubu[h_o]; bsubv_o=bsubv[h_o];
        sH_o = sqrtS_H[j];
    }
...
// §8.10 split prototype entry point: two kernels instead of one. The R/Z and
// lambda launches use the same (128, (nZnT,ns)) shape as the monolith and write
// disjoint force families, so the combined output covers all 16 arrays.
template <typename T>
void computeForcesSplit(const FourierPlan<T>& fp, const GridParams<T>& p,
    ...
    dim3 block(128);
    dim3 grid((p.nZnT + 127) / 128, p.ns);
    rzForcesKernel<T><<<grid, block, 0, stream>>>(
        geometryParityViews(fp, p), baseGeometryHalfViews(mw, p),
        magneticFieldViews(mw, p), radialProfileViews(rp),
        forceParityViews(fp, p), p.ns, p.nZnT, rp.delta_s);
    lambdaForcesKernel<T><<<grid, block, 0, stream>>>(
        geometryParityViews(fp, p), baseGeometryHalfViews(mw, p),
        magneticFieldViews(mw, p), radialProfileViews(rp),
        forceParityViews(fp, p), p.lamscale, p.ns, p.nZnT);
    cumes::check_cuda(cudaGetLastError(), "forces split kernels");
}
