...
The initial implementation covers native CUDA fixed boundaries in float and
double. `test_asymmetric` checks inverse geometry, analytic angular derivatives,
...
WebGPU and free-boundary requests currently reject `lasym=true` explicitly
while their coupling is extended. Newton corrections, the retained tangent
