...
class SpectralView {
 public:
  ...
  __host__ __device__ T& operator()(SpectralComponent c, int mode, int surface) {
    return data_[static_cast<int>(c) * mnmax_ * ns_ + mode * ns_ + surface];
  }
  __host__ __device__ const T& operator()(SpectralComponent c, int mode,
                                          int surface) const {
    return data_[static_cast<int>(c) * mnmax_ * ns_ + mode * ns_ + surface];
  }
...
class RealFieldView {
 public:
  ...
  __host__ __device__ RealFieldView(T* data, int surfaces, int ntheta,
                                    int nzeta)
  ...
  __host__ __device__ const T& operator()(int surface, int zeta, int theta)
      const {
    return data_[surface * ntheta_ * nzeta_ + zeta * ntheta_ + theta];
  }
