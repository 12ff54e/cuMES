...
// device operators they are a host-side build step. The operator OWNS the 11
// full/half-grid radial arrays directly (carved from the stage arena) and
// exposes them as a typed RadialProfileViews bundle plus the radial grid
// spacing. (Migration step 13.3: the legacy RadialProfiles struct +
...
class Profiles {
 public:
  ...
  RadialProfileViews<T> profile_views() const {
    ...
    v.iota_F = d_iota_F_; v.phip_F = d_phip_F_;
    v.chi_F = d_chi_F_; v.sqrtS_F = d_sqrtS_F_;
    v.iota_H = d_iota_H_; v.pres_H = d_pres_H_; v.phip_H = d_phip_H_;
    v.dVds_H = d_dVds_H_; v.sqrtS_H = d_sqrtS_H_;
    v.curr_H = d_curr_H_; v.chip_H = d_chip_H_;
    return v;
  }
