...
// descent kernels — not transform scratch, so it does not belong in the
// FourierPlan (blueprint §5.1: transforms own only tables/plans/scratch). It is
...
// fourierCreate produced). With `arena == nullptr` each array is its own
