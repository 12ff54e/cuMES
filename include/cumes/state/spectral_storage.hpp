...
// separate `[mode][surface]` array) is gone (migration step 13.3).
// SpectralStorage co-locates those twelve arrays into two component-major slabs
// — `state` (6*mnmax*ns) and `velocity` (6*mnmax*ns) — in the exact order Rcc
// Zsc Lsc Rss Zcs Lcs; family_ptr()/velocity_family_ptr() return the 12
...
class SpectralStorage {
   public:
    ...
    void allocate(int ns, int mnmax) {
        ...
        // and the zeroed velocities of Prolongation::enqueue.
        state_.zero();
        velocity_.zero();
    }
    ...
    // Owning-buffer access for the single-copy checkpoint/restore in the
    // solver.
