// accumulation.hpp — the norm/reduction accumulator type (blueprint §8.8,
// §8.12).
//
// The mixed-float policy accumulates residual/norm reductions in double even
// when the state/geometry is float, so a ~15k-term summation (mnmax*ns) does
// not lose precision to per-add float rounding. NormAccum<T> encodes that
// default:
//
//   float  state -> double accumulation  (mixed-float, Class B for the float
//                                         build only)
//   double state -> double (== T, so the verified double build is
//   bit-identical)
//
// The *stored* output scalar stays T — the benefit is the summation width, not
