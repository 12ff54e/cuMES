// axisymmetric_operator.hpp — the axisymmetric spectral backend (blueprint §8.5).
//
...
class AxisymmetricOperator : public SpectralOperator<T> {
 public:
  ...
  // written as zero (no n=0 basis function); axis/LCFS rules match the generic
  // recover (axis keeps m=0 frcc only, LCFS keeps λ only).
