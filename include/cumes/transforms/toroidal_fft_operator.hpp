...
class ToroidalFftOperator : public SpectralOperator<T> {
   public:
    ...
    // the stage-owned `rs` this operator was constructed with.
    void inverse(SpectralView<const T, PhysicalStateDomain> coeff,
                 bool do_combine,
    ...
    void dealias_bandpass(const T* gConEff,
                          const T* tcon,
                          const T* faccon,
                          T* gCon,
                          cudaStream_t stream = 0);
   ...
   private:
    ...
    typename FftTraits<T>::Complex* d_zeta_spectra_ =
        nullptr;                // [12*mpol*ns][nz2]
    ...
    const DeviceModeTable* mt_ =
        nullptr;  // non-owning (stage/resolution-owned)
};
