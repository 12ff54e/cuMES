...
// solver needs: the fused inverse carries the xmpq-weighted rCon/zCon
// constraint reconstruction (the generic backend fuses them into the inverse
// accumulate; the axisymmetric backend runs a direct-poloidal rzCon kernel
// right after its synthesis), and the de-alias bandpass is dispatched through
// the same interface (generic: compact cuFFT round trip; axisymmetric: direct
// poloidal).
//
...
class SpectralOperator {
   public:
    ...
    // and (fused) the xmpq = m(m-1)-weighted constraint reconstruction
    // rCon/zCon (full real-space fields, no parity split / scalxc — blueprint
    // §4.8). `rCon`
    ...
    // backend runs the compact cuFFT round trip; the axisymmetric backend a
    // direct poloidal sum.
    virtual void enqueue_dealias(RealFieldView<const T> gConEff,
                                 const T* tcon,
                                 const T* faccon,
